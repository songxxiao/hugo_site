...
```{r f}
cludta <- dta %>% 
  select(lfatd1,lfatd2,lfatd3,catatd1,catatd2,catatd3,age)

clu <- dta %>%
  select(S5,S10B,edu_stage,child)
...
summarize(平均年龄 = mean(age),
           平均教育年限 = mean(eduy)),digits=2,caption='分聚类类别描述性统计',align='c')
...
tree.fit <- rpart(clusters ~ edu_stage +S5+age_stage+ D5+S10B+child+lfatd1+lfatd2+lfatd3	+catatd1+catatd2,data = dta,method = "class",parms = list(split="gini"),control=cont,na.action = na.omit)

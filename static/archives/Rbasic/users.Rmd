...
```{r a}
library(tidyverse)
dt.car <- read.csv("C:/Users/xsong/Desktop/table/DT3.csv")
...
```{r b}
dt.car$S10C <- as.character(dt.car$S10C)
#dt.car$S3 <- as.numeric(dt.car$S3)

dta <- dt.car %>%
  select(S5,S3,S6__1__open,D5,D6,D3,S10,S10C,Q7__1:Q7__35,A0__1:A0__27,D12A,city_tier) %>% #筛选列
  ...
  mutate(cityle=recode(S3,'成都'='一线','上海'='一线','广州'='一线','南京' ='二线','惠州'='二线', '郑州' ='二线','唐山'='三线', '常州'='三线', '临沂'='三线','北京'='一线城市'),
    ...
    eduy=recode(edu,'初中'=9,
         ...
         child=recode(D6,'有1个孩子'='有孩子',
                      ...
                      '没有孩子'='没有孩子',
                      .default='没有结婚')) %>% #重编码
  select(edu,eduy,cityle,everything()) 
...
clu <- dta %>%
  select(S10C,child,S5,S10C,age_stage)

cludta <- cbind(clu,cludta)

cludta$S5 <- as.factor(cludta$S5)
cludta$S10C <- as.factor(cludta$S10C)
...
#dta <-subset(dta,edu_stage!="NA")
#dta <-subset(dta,S5!="NA")
#dta <-subset(dta,S10C!="NA")
...
knitr::kable(table(dta$S10C,dta$clusters),caption = '聚类类别和车系的交互表')
mar <- table(dta$S10C,dta$clusters)
...
knitr::kable(table(dta$city_tier,dta$clusters),caption = '聚类类别和城市类别的交互表')
mar <- table(dta$city_tier,dta$clusters)
...
tree.fit <- rpart(clusters ~ edu_stage+S5+age_stage+ D5+S10C+child+Q7__23+D12A,data = dta,method = "class",parms = list(split="gini"),control=cont,na.action = na.omit)

...
为了讨论学习成绩的门槛效应，我们将数学成绩是否位于所有学生的前25\%(是=1，否=0)单独划分为一个分类变量建立分类树。图\@ref(fig:b) 展示了标准化总成绩的密度直方图估计。可以发现，标准化总成绩基本上呈现了正态分布。

图\@ref(fig:a) 展示了以随机抽取的30所学校为分组变量，原始数学成绩为因变量的箱线图。
...
[R语言移除缺失值 NA.RM](http://www.mamicode.com/info-detail-1402416.html)

```{r b,message = F,comment = NA,warning = F,echo = F,fig.align='center', out.width='100%',fig.cap='标准化总成绩的直方图(分年级)',fig.asp=0.4}
library(haven)
library(dplyr)
`%>%` <- magrittr::`%>%`
library(rpart)
...
```{r a,message = F,comment = NA,warning = F,fig.align='center',out.width ='100%',fig.cap='随机抽取30所学校的数学成绩差异'}
setwd("E:/edu/data2")
ceps <- read_dta("E:/edu/data2/ceps.dta")
#抽出30所学校，作成绩箱线图
ceps1 <- ceps %>% 
...
```{r others,message = F,comment = NA,warning = F,eval= F,fig.align='center',out.width ='100%',fig.cap='随机抽取30所学校的数学成绩差异'}
#独生子女
...
图\@ref(fig:s) 展示了总成绩的回归结果。由于可能存在七年级和九年级学生考试总成绩的不同的问题[^1]。此处将九年级子样本单独进行分析。

[^1]:七年级可能会采取单科100分制，而九年级按照中考模式更可能采取单科150分制。不同地域之间也存在差别。


# Lasso回归

```{r lasso,message=F,comment=NA,warning=F,message = F}
setwd("E:/edu/data2")
ceps <- read_dta("E:/edu/data2/ceps.dta")
ceps <- ceps %>% 
...
lasso_mod = glmnet(x_train, 
                   ...
                   alpha = 1)
plot(lasso_mod)
plot(lasso_mod,xvar="lambda",label=T)  

```


```{r re,message=F,comment=NA,warning=F,message = F}
#建立回归树
...
```{r s,,comment=NA,warning=FALSE,out.width ='100%',fig.cap="总成绩的回归树",fig.align='center'}
tree$cptable #复杂度参数表
rpart.plot(tree)
```

图\@ref(fig:fig1) 展示的是数学原始成绩是否位于前25\%的分类决策树。首先，我们在备选的约50个自变量中纳入学校哑变量。仅考虑家庭、个体、班级因素对学生学习成绩的影响。通过划分训练集和测试集的方式计算模型的预测错误率。检验模型的泛化能力。之后对原始的树进行剪枝，剪枝后的分类树如图\@ref(fig:fig5) 所示。

[R语言表格教程](https://www.jianshu.com/p/08c28c5c55a7)

```{r fig1,message = F, comment = NA,warning =F,fig.align='center',out.width ='100%',fig.cap='未剪枝的分类树'}
set.seed(1212323) #设置随机数种子

#简单交叉验证划分测试集和训练集
sampled <- sample(1:nrow(ceps),nrow(ceps)*0.3,replace=F) #抽取30%样本作为验证集
...
```{r fig6, message = F, comment = NA,warning = F,fig.align='center',out.width ='100%',fig.cap='剪枝后分类树的ROC曲线'}
...
#ROC 曲线
plot(modelroc, print.auc=T, auc.polygon=T, max.auc.polygon=T,auc.polygon.col="skyblue", print.thres=T,xlab ='伪正类率',ylab='真正类率')
...
从图\@ref(fig:fig5) 发现，对因变量分类有显著贡献的变量有学校哑变量、对孩子未来的信心、上兴趣班费用总计、与学生交流时间。可见学校层面的特征对数学成绩的影响非常强。

在初次拟合分类树之后，对分类树进行剪枝。通过第一棵分类树的结果，我们选取预测误差(即表中的xerror通过交叉验证获得)最小的CP值带入到下一个模型中，最终的验证集错误率在25\%左右。

## [交叉验证](http://topepo.github.io/caret/train-models-by-tag.html#implicit-feature-selection)

使用`caret`包进行10次10折交叉验证,同样使用CART算法。图\@ref(fig:fig7) 展示了训练后模型的ROC曲线。

```{r fig7,cache=T,message = F,comment = NA,warning=F,fig.align='center',out.width ='100%',fig.cap='训练后模型的ROC曲线和AUC值'}
...
ceps <- na.omit(ceps)
#10次10折交叉验证
repeatedcv <-trainControl(method ="repeatedcv", number =10,repeats =10, savePredictions=T)
...
```{r fig8,cache = T,message = F,comment = NA,warning=F,fig.align='center',out.width ='100%',fig.cap='分类树的复杂度参数与准确率的关系'}
plot(rpartFit,xlab ='复杂度参数',ylab='准确率(重复交叉验证)')
```

## 随机森林

作为`Bagging`方法的扩展，随机森林采用决策树作为基分类器。随机森林算法不仅做到了随机样本扰动，还引入了随机属性扰动。这使得随机森林比`Bagging`法具有更强的泛化能力。具体而言，随机森林首先使用`Bootstrap`法同时对数据行和列进行重复抽样。其次，每一次抽出的子样本都使用一棵决策树拟合。在构建每一棵决策树的过程中，可以无需剪枝。由于属性和样本的随机性，随机森林并不会遇到过拟合的问题。当需要对样本进行预测时，可将样本属性值带入每一棵决策树中计算结果，并通过投票法确定被选择最多的那个类。




```{r randomForest,message = F,comment = NA,warning = F,fig.align='center',out.width ='100%',fig.asp=1.2,fig.cap='变量重要性'}

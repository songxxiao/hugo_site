...
```{r,message = F, comment = NA,warning = F,fig.asp=0.4}
set.seed(222)
...
plta <- ggplot(Grunfeld, aes(value,inv)) +
  geom_point(aes(color=factor(firm)),size=2) +
  geom_smooth(method='lm',se=F,color='red') +
  theme(legend.position = 'none')+
  theme_bw()  

moda <- lm(inv ~ value + factor(firm),Grunfeld)
grid <- Grunfeld %>%
data_grid(value,firm) %>%
gather_predictions(moda)
          
pltb <- ggplot(Grunfeld, aes(value,inv)) +
  geom_point(aes(color=factor(firm)),size=2) +
  geom_line(data=grid,aes(value,pred,color=factor(firm),group=factor(firm)),size=0.75) +
  theme(legend.position = 'none')+
  theme_bw() 
plot_grid(plta,pltb,ncol = 2)
```
...
stargazer::stargazer(ols,fe,tw,
          ...
          dep.var.labels=' ',
          align=T,
          type='html',column.labels=c('线性回归','组内估计','双重固定'),

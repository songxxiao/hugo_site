...
```{r 1,message = F, comment = NA,warning = F,cache = T,fig.align='center',out.width ='80%',fig.asp=0.7,fig.cap='`ggplot2`按图层绘图。为防止散点堆叠，图中散点经过抽样。lowess曲线是根据原始数据绘制。不同图层可使用不同数据。'}
...
modelb <- ggplot() +
  ...
  geom_line(data=grid, aes(y = pred,x=x,group=z),size=0.75,color='blue') +
  ylim(0.3,2.1)+
  theme(legend.position='none')

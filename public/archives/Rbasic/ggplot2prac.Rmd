...
```{r,include=F}
knitr::opts_chunk$set(comment=NA,error=T,message = F,warning = F,fig.align='center',out.width ='100%',fig.showtext= T)
```
...
```{r,out.width ='80%',fig.asp=0.6,fig.cap='`ggplot2`按图层绘图。为防止散点堆叠，图中散点经过抽样。lowess曲线是根据原始数据绘制。不同图层可使用不同数据。位于`ggplot`下方的图层将被堆叠在上方。'}
...
```{r,fig.asp=0.8,fig.cap='散点图的颜色代表不同组。左上:简单OLS拟合。右上:令每个组斜率相同、截距不同(固定效应)。相当于将组别变量`z`作为控制变量。原本的正相关相关关系颠倒为负相关。左下:对每个组分别拟合线性模型。各组斜率不同，截距也不同。'}
set.seed(91)
...
modelc <- ggplot(dt, aes(x, y,group=z)) +
  ...
  geom_smooth(method='lm',se=F)+
  theme(legend.position='none')
plot_grid(modela,modelb,modelc,ncol = 2)
```
...
out12 <- ggplot(china_data, aes(long,lat, group = group,fill= out12)) +
...
in12 <- ggplot(china_data, aes(long,lat, group = group,fill= in12)) +
  geom_polygon(colour="brown",size=0.2) +
  scale_fill_gradientn(colours = c("yellow", "red"),na.value = "white",
                       breaks = c(10,20,30,40),limits=c(0,43), guide = "legend") +  
  coord_map("polyconic") +  
...
out14 <- ggplot(china_data, aes(long,lat, group = group,fill= out14)) +
  geom_polygon(colour="brown",size=0.2) +
  scale_fill_gradientn(colours = c("yellow", "red"),na.value = "white",
                       breaks = c(10,20,30,40),limits=c(0,43), guide = "legend") + 
  coord_map("polyconic") + 
...
in14 <- ggplot(china_data, aes(long,lat, group = group,fill= in14)) +
  geom_polygon(colour='brown',size=0.2) +
  scale_fill_gradientn(colours = c("yellow", "red"),na.value = "white",
                       breaks = c(10,20,30,40),limits=c(0,43), guide = "legend") +  
  coord_map("polyconic") + 
...
out16 <- ggplot(china_data, aes(long,lat, group = group,fill= out16)) +
  geom_polygon(colour="brown",size=0.2) +
  scale_fill_gradientn(colours = c("yellow", "red"),na.value = "white",
                       breaks = c(10,20,30,40),limits=c(0,43), guide = "legend") + 
  coord_map("polyconic") +   
...
in16 <- ggplot(china_data, aes(long,lat, group = group,fill= in16)) +
  geom_polygon(colour="brown",size=0.2) +
  scale_fill_gradientn(colours = c("yellow", "red"),na.value = "white",
                       breaks = c(10,20,30,40),limits=c(0,43), guide = "legend") +  
  coord_map("polyconic") +     
...
```{r,fig.cap='中国土地流转的空间分布。北京、天津由于农户过少而设为缺失。CFPS未调查的省份数据缺失。土地流转发生率=当年参与土地流转的农户数/当年农户总数。数据经过加权处理，具有全国代表性。'}
...
ggplot(long, aes(long,lat,group = group,fill= value)) +
  geom_polygon(colour='brown',size=0.2) +
  scale_fill_gradientn(colours = c('navyblue','yellow'),na.value = 'white',breaks = c(40,30,20,10),limits=c(0,43), guide = 'legend') +
  coord_map('polyconic') +
  facet_wrap(~variable,nrow = 3) +
  labs(x='经度',y='纬度',fill = '土地流转发生率') +
  theme_bw()

ggplot(long, aes(long,lat,group = group,col= value,fill= value)) +
  geom_polygon(col='brown',size=0.2) +
  scale_fill_viridis_c(na.value = "white", guide = "colourbar", aesthetics = "fill")+
  coord_map('polyconic') +
  facet_wrap(~variable,nrow = 3) +
  labs(x='经度',y='纬度',fill = '土地流转发生率') +
  theme_bw()
...
ggplot(china_map,aes(long,lat)) + 
  geom_polygon(aes(group = group),
               fill="white",color="black",
               size=0.1) + 
  geom_point(data = center,
             aes(long,lat, size=freq),
             color='red') +
  coord_map('polyconic') +
  labs(x='经度',y='纬度') +
  theme(legend.title=element_blank()) + 
...
earthquake <- read_csv("E:/MaLearning/earthquake20190630.csv", locale = readr::locale(encoding = "gb18030"))

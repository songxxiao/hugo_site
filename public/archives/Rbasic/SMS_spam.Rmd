...
#对于新数据集。判断DTM的每一列是否在训练集中出现。出现保留，没出现剔除，补充空列。
...
sample = colnames(smmat) %in% colnames(newdt)
smmat[,sample] = 1 # 提取在训练集中出现的列，将对应的值重编码为1
...
test_dtmx <- function(string){ # 得到新数据的DTM
test_dtm = VCorpus(VectorSource(string)) %>% 
  DocumentTermMatrix(., control = list(tolower = T,
                                      removeNumbers = T,
                                      stopwords = T,

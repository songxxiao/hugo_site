...
```{r pack,comment = NA,warning = F,message = F,fig.align='center',results = "hide"}
# 加载包
library(survival)
library(haven) 
library(ggplot2)
...
```{r a,comment = NA,warning = F,message = F,fig.align='center',include = F}
...
又称乘积极限法(Product-Limit Method), 公式如下：
...
```{r b,comment = NA,warning = F,message = F,fig.align='center',out.width ='100%'}
table(cwg$event)
...
```{r sur1, echo=F, fig.cap=" ", out.width = '100%',fig.align='center'}
knitr::include_graphics("/img/survival/life.jpg")
```
生存函数的Kaplan–Meier estimator就是$nPx$列的累乘。

```{r c,comment = NA,warning = F,message = F,fig.align='center',out.width ='100%'}
summary(km_fit, times = c(0.2*(1:25))) #这个表和生命表等价
```

```{r sur2, echo=F, fig.cap=" ", out.width = '100%',fig.align='center'}
knitr::include_graphics("/img/survival/life2.jpg")
```

```{r cw,comment = NA,warning = F,message = F,fig.align='center',out.width ='100%'}
...
```{r cox,comment = NA,warning = F,message = F,fig.align='center',out.width ='100%'}
...
```{r areg,comment = NA,warning = F,message = F,fig.align='center',out.width ='100%',fig.cap='协变量作用如何随时间变化'}

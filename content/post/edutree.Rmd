...
```{r bust,message = F,comment = NA,warning = F,message = F,echo = F,fig.align='center', out.width='100%',fig.cap='标准化总成绩的直方图'}
setwd("E:/edu/data2")
library(haven)
library(tidyverse)
library(rpart)
...
为了直观地描述不同学校与学习成绩的差异，图2展示了以学校为分组变量，原始数学成绩为因变量的箱线图。

```{r a,message = F,comment = NA,warning = F,message = F,echo = T,fig.align='center',out.width ='100%',fig.cap='随机抽取30所学校的数学成绩差异'}
setwd("E:/edu/data2")
ceps <- read_dta("E:/edu/data2/ceps.dta")
#抽出30所学校，作成绩箱线图
ceps1 <- ceps %>% 
...
首先我们将展示总成绩的回归结果。由于七年级和九年级学生考试总分的不同。我们将九年级子样本单独进行分析。
...
```{r c ,message = F, comment = NA,warning = F,message = F,include=T,fig.align='center',out.width ='100%',fig.cap='未剪枝的分类树\\label{fig3}'}
...
```{r d ,message = F, comment = NA,warning = F,message = F, include = T,fig.align='center',out.width ='100%',fig.cap='ROC曲线'}
...
使用`caret`包进行10次10折交叉验证,同样使用CART算法。

```{r caret,cached = T,message = F,comment = NA,warning = F,message = F,include=T,fig.align='center',out.width ='100%'}

...
接下来，我们对分类树进行剪枝。再次计算剪枝后的决策树的测试集错误率。

```{r d ,message = F,prompt=T, comment = NA,warning = F,message = F, include = T}

cont <- rpart.control(minsplit=5,maxcompete=100,xval=10,maxdepth=30,cp=0.02) #设置参数
...
y.pr <- predict(cltree4,test,type="prob") #预测验证集概率

pr <- y.pr[,2] 
test <- cbind(test,pr)


test$yhat <- ifelse(test$pr >0.5,'Yes','No') #大于0.5设置为"是"
attach(test)
tab <- table(yhat,matgreat) 
treeerror <- (sum(tab)-sum(diag(tab)))/sum(tab)
tab

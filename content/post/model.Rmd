...
```{r,message=F,warning=F,error=T,comment=NA,fig.align='center',out.width='100%',fig.asp=0.5,fig.cap='左图：$y = 2 + x + 0.5 z + 0.5 x z + \\epsilon$。右图：$y = 2 + x + 0.5 z + \\epsilon$。圆圈面积表示因变量$y$的大小。'}
...
```{r,message=F,warning=F,error=T,comment=NA,fig.align='center',out.width='100%',fig.asp=0.6,fig.cap='使用R自带作图工具绘图。作图：设置不同贷款进行LOWESS回归。右图：Bootstrap重抽样200次的结果。'}

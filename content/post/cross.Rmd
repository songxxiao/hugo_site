...
```{r 1,comment = NA,warning = F,message = F,fig.align='center'}
`%>%` <- magrittr::`%>%`
library(caret)
library(readr)
library(tidyr)
...
```{r 2,comment = NA,warning = F,message = F,fig.align='center'}
...
```{r cv,comment = NA,warning = F,message = F,fig.align='center',out.width='70%',fig.asp=1.2} 
...
## 留一交叉验证(leave-one-out cross validation)
```{r 3,comment = NA,warning = F,message = F,fig.align='center'}
...
```{r 4,comment = NA,warning = F,message = F,fig.align='center',fig.cap='K值和准确率的关系',out.width='65%'}
...
```{r 5,comment = NA,warning = F,message =F,fig.align='center',fig.cap='ROC曲线',out.width='65%'}
